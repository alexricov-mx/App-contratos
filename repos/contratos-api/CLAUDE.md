...
3. `../App-Contratos/docs/planes/plan-api.md` — **tu plan de trabajo**
...
- .NET 10, Minimal APIs, **Vertical Slice Architecture**: `Features/<Area>/<CasoDeUso>/{Endpoint,Request,Validator,Handler}.cs`. Sin servicios/repositorios genéricos; lo compartido solo en `Common/`.
- EF Core + Npgsql, UUID generados en .NET, `snake_case`, `jsonb` para Delta y alcance, columna `version` como token de concurrencia, borrado lógico.
- Esquema **compatible con PostgreSQL administrado**: sin extensiones ni funciones de superusuario.
- La conexión a BD se obtiene **siempre** de `IProveedorConexion` (nunca `appsettings` directo) para permitir el cambio en caliente.
- Errores como ProblemDetails con `code` de `contrato-api.md` §1.
- Migraciones: solo se agregan; no se editan migraciones ya integradas a `main`.
- Secretos solo por variables de entorno / volumen; nada en la imagen ni en el repo.
